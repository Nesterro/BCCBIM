    ...
    public class HeatLossEngine
    {
        ...
        public List<ExternalDefinition> GetOrCreateHeatLossDefinitions(HeatLossCalculationResult result)
        {
            ...
            try
            {
                ...
                // 1. Try using existing active shared parameter file in Revit
                try
                {
                    origSharedFile = _doc.Application.SharedParametersFilename;
                ...
                // 2. If active file is missing or failed to open, create clean file in User Temp
                if (defFile == null)
                {
                    ...
                    File.WriteAllText(sharedFilePath, sb.ToString(), Encoding.Unicode);
                ...
                if (defFile != null)
                {
                    ...
                    if (group == null)
                    {
                        try { group = defFile.Groups.Create("BIMBCC_Теплопотери"); } catch { }
                    }
                    ...
                    if (group != null)
                    {
                        ...
                        foreach (var item in paramsToEnsure)
                        {
                            ...
                            if (def == null)
                            {
                                try
                                {
                                    ExternalDefinitionCreationOptions opt = new ExternalDefinitionCreationOptions(item.name, item.forgeType)
                                    {
